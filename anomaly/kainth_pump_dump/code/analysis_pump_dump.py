...
def analyse_symbol(f_path, volume_thresh, price_thresh, window_size=24, candle_size='1h'):
    ...
    price_dump_mask, price_dump_df = find_price_dumps(df, window_size)

    vol_dump_mask, vol_dump_df = find_volume_dumps(df, window_size)

    # find price and volume spikes
    vp_combined_mask = vol_mask & price_mask
    vp_combined_df = df[vp_combined_mask]
    # num_vp_combined_rows = get_num_rows(vp_combined_df)

    # coinciding price and volume spikes for alleged P&D
    vp_combined_rm = rm_same_day_pumps(vp_combined_df)
    num_alleged = get_num_rows(vp_combined_rm)

    # find price and volume spikes with dumps afterwards
    final_combined_mask = vol_mask & price_mask & price_dump_mask
...
# returns a exchange_name,symbol_name,dataframe
...
# Any of the data collected from the pull_xchange_data.py will work if plugged into the file path
# The basic parameters can be changed here
analyse_symbol(f_path='..//data/kucoin/kucoin_ACAT-BTC_[2019-11-20 00.00.00]-TO-[2019-11-28 18.00.00].csv',
               volume_thresh=4,
               price_thresh=1.05,
               window_size=12,
               candle_size='1h')
