...
--provider-uri http://zecwallet:beq9dvhxqyh@localhost:8327 --chain zcash \
...
#  --provider-uri http://zecwallet:beq9dvhxqyh@localhost:8327 \
...
#def get_block_range_for_date():
    #os.system(f'bitcoinetl get_block_range_for_date --provider-uri http://zecwallet:beq9dvhxqyh@localhost:8327 --date={date_entry}')

def export_all():
    os.system(f'bitcoinetl export_all --provider-uri http://zecwallet:beq9dvhxqyh@localhost:8327 --start {start_date} --end {end_date}')
