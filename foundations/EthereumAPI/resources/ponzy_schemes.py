...
def serialize_transaction(ponzi_Anomaly_data):
...
class GetPonziAnomalyDataEndpoint(Resource):
    def get(self):
        ...
        try:
            # perform the query
            ponzi_Anomaly_data = db_session.query(PonziAnomaly).order_by(PonziAnomaly.id.asc())
            if ponzi_Anomaly_data is not None and len(list(ponzi_Anomaly_data)) != 0:
                ponzi_Anomaly_list = []
                for ponzi_Anomaly in ponzi_Anomaly_list:
                    ponzi_Anomaly_list.append(serialize_transaction(ponzi_Anomaly))
                response = {
