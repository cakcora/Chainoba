...
The function will create csv file (in clustering folder, called CommonSpending)
 in each row it has the transaction hash and the input addresses of the transaction.
...
In2017 16th IEEE International Conference on Machine Learning and Applications (ICMLA) 2017 Dec 18 (pp. 461-466). IEEE.
    ...
    for x in range(NumberOfTxsTo):
        ...
        for row in arr:
         ...
         # check if number of input addresses greater than one, and one output address then they are belong to same user.
